...
Folqen now includes a Connection Wizard in `/settings`. It asks for Google Drive, OpenAI, n8n, and social channel setup details inside the app. Sensitive values are encrypted before being saved to Supabase using the server credential vault key. Folqen must never ask for raw social media passwords; social platforms should use OAuth/API setup when those adapters are built.
