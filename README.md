...
This deployment is live for testing the shell, login screen, protected-route behavior, health/status endpoints, database-backed authentication, settings, approvals, audit logs, persistent mock agent chat, route-specific protected pages, manual posting package generation/download, metadata-only file registration, safe mock-agent draft creation, provider setup panels, provider approval requests, Google Drive private-storage adapter fallback behavior, and Supabase-backed dashboard/workspace data. It is not a complete production MVP yet because live n8n secrets, Google Drive OAuth/storage credentials, OpenAI key/payment approval, platform integrations, and real media workflows are still pending.
...
Google Drive is now the planned cloud storage provider in the settings/provider UI, and the server-only upload adapter exists. It stays in metadata-only fallback mode until Google OAuth credentials, a refresh token, and a private Drive folder id are added through server-only environment variables. The n8n tab can embed the self-hosted n8n UI after `ORACLE_N8N_INSTANCE_URL` is configured and the n8n instance allows iframe embedding. OpenAI model preference is saved through the settings dropdown, but real OpenAI calls remain blocked until an API key is configured and paid-tool approval gates are intentionally enabled. Live social integrations, real n8n workflows, real AI calls, media rendering, and real publishing are not implemented yet.

Folqen now includes a Connection Wizard in `/settings`. It asks for Google Drive, OpenAI, n8n, and social channel setup details inside the app. The `/platforms` tab also includes a social-only connection wizard for YouTube, Instagram, Facebook, Snapchat, and Threads. Sensitive values are encrypted before being saved to Supabase using the server credential vault key. Folqen must never ask for raw social media passwords; social platforms should use OAuth/API setup when those adapters are built.
...
- Connection note: `/settings` includes a Connection Wizard for encrypted credential intake. Saving credentials does not publish, spend credits, or run workflows.
