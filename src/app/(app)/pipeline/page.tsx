...
export default async function PipelinePage() {
  ...
  const [overview, workflowRuns] = await Promise.all([
    ...
    <div className="section-space pb-8">
      <Card className="panel-soft">
        <CardContent className="py-5">
          <h1 className="font-display text-2xl font-semibold tracking-[-0.03em]">Pipeline</h1>
          <p className="mt-2 max-w-3xl text-sm leading-7 text-muted-foreground">
            Track progress only from real workflow runs. No synthetic queue or stage activity is shown here.
          </p>
        </CardContent>
      </Card>
      {workflowRuns === 0 ? <SurfaceEmptyState model={model} /> : null}
    </div>
  );
}
