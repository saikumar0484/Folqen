...
export function resolveMediaMutationAccess(input: { user: CurrentUser | null; safetyError?: { status: number; error: string } | null }) {
  if (!input.user) {
    return { ok: false as const, status: 401, error: "Login required." };
  }

  if (!canCreateDraftContent(input.user)) {
    return { ok: false as const, status: 403, error: "Only admins and operators can run media pipelines." };
  }
...
export function resolveMediaReadAccess(user: CurrentUser | null) {
  if (!user) {
    return { ok: false as const, status: 401, error: "Login required." };
  }
