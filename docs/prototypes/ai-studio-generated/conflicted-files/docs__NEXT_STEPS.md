...
1. Implement login/logout and session handling.
2. Add role model (`admin`, `operator`, `viewer`) and middleware protection for app routes.
3. Add password hashing and secure auth flow scaffolding.
4. Add security headers and initial audit log helper.
5. Seed default admin user (`admin@example.com` / `ChangeMe123!`) with README warning.
6. Verify lint/typecheck/test/build and fix all issues.

## Human decisions needed

- None for initial Phase 3 implementation (credentials not required yet).

## Credentials needed

- None.

## Risky actions coming next

- Auth/security route protection and session logic.
