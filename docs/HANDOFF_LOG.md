...
May 13, 2026. Latest completed slice is Browser Operations Department, Safe Preview Deployment, preview-only demo auth, and repository audit cleanup. Folqen now has `GET /api/browser-ops/overview`, `POST /api/browser-ops/session`, `POST /api/browser-ops/workflow`, `POST /api/browser-ops/control`, `/browser-operations`, `GET /api/deployment/preview`, preview deployment diagnostics in `/infrastructure`, `deploy/.env.preview.example`, docs for Browser Operations and safe preview deployment, a signed-session preview demo login for viewing `/dashboard` without `DATABASE_URL`, and `docs/REPOSITORY_AUDIT_2026-05-13.md`. The generated prototype scaffold under `docs/prototypes/ai-studio-generated/**` was removed after audit proof, and unimported legacy components were moved to `archive/review-required/`. Browser Operations is dry-run only and never launches Playwright, contacts websites, uses cookies/account sessions, uploads files, scrapes, or automates accounts. Preview demo auth only works when `PREVIEW_DEMO_AUTH=true`, preview safe mode, forced dry-run, and disabled execution flags are all present. No production credentials, live migration application, production env, paid tools, unrestricted provider activation, unrestricted GPU execution, video generation, live ComfyUI request, FFmpeg process spawn, live browser execution, platform account access, live platform analytics API read, scraping, public publishing, n8n execution, autonomous optimization execution, autonomous self-improvement, prompt mutation, workflow mutation, autonomous media retry, or unrestricted media generation was enabled.
