...
- Controlled live execution has exactly three live-capable code path families: Gemini Research content ideation at `/api/live-execution/research/ideation`, governed Research workflows at `/api/live-execution/research/workflows`, and governed Content workflows at `/api/live-execution/content/workflows`. They remain unreachable without every persisted activation, real approval, credential, budget, sandbox, governance, provider health, and kill-switch gate. No autonomous retries or fallback providers are allowed.
