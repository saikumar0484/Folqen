...
May 13, 2026. Latest completed slice is Browser Operations Department and Safe Preview Deployment. Folqen now has `GET /api/browser-ops/overview`, `POST /api/browser-ops/session`, `POST /api/browser-ops/workflow`, `POST /api/browser-ops/control`, `/browser-operations`, `GET /api/deployment/preview`, preview deployment diagnostics in `/infrastructure`, `deploy/.env.preview.example`, and docs for Browser Operations and safe preview deployment. Browser Operations is dry-run only and never launches Playwright, contacts websites, uses cookies/account sessions, uploads files, scrapes, or automates accounts. No credentials, live migration application, production env, paid tools, unrestricted provider activation, unrestricted GPU execution, video generation, live ComfyUI request, FFmpeg process spawn, live browser execution, platform account access, live platform analytics API read, scraping, public publishing, n8n execution, autonomous optimization execution, autonomous self-improvement, prompt mutation, workflow mutation, autonomous media retry, or unrestricted media generation was enabled.
