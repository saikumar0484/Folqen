...
Current save point: May 13, 2026, after the Browser Operations Department and Safe Preview Deployment profile. The latest live deployment remains `https://folqen.vercel.app`; no production deploy, live database migration application, credential change, paid tool activation, production provider activation, live AI provider execution, live embeddings, live browser execution, scraping, account automation, live ComfyUI request, FFmpeg process execution, unrestricted GPU execution, platform account access, live platform analytics API read, publishing change, autonomous retry, autonomous optimization execution, prompt mutation, self-improvement mutation, workflow mutation, queue mutation, or n8n execution was enabled in this slice. Browser Operations is dry-run only, and a live-capable local-worker thumbnail endpoint still requires explicit env flags, worker endpoint/secret, real approval, budget/quota validation, and kill switches off before any render can happen.
