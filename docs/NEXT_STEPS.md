...
Current save point: May 13, 2026, after the Browser Operations Department, Safe Preview Deployment profile, and preview-only demo auth for Vercel dashboard visualization without a preview database. The latest live deployment remains `https://folqen.vercel.app`; no production deploy, live database migration application, production credential change, paid tool activation, production provider activation, live AI provider execution, live embeddings, live browser execution, scraping, account automation, live ComfyUI request, FFmpeg process execution, unrestricted GPU execution, platform account access, live platform analytics API read, publishing change, autonomous retry, autonomous optimization execution, prompt mutation, self-improvement mutation, workflow mutation, queue mutation, or n8n execution was enabled in this slice. Browser Operations is dry-run only, preview demo auth only works with preview safe mode and forced dry-run, and a live-capable local-worker thumbnail endpoint still requires explicit env flags, worker endpoint/secret, real approval, budget/quota validation, and kill switches off before any render can happen.
