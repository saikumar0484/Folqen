...
Current save point: May 13, 2026, after the first real live Gemini Research ideation capability update. The latest live deployment remains `https://folqen.vercel.app`; no production deploy, live database migration application, credential change, paid tool activation, production provider activation, live AI provider execution, live embeddings, live ComfyUI execution, GPU execution, FFmpeg rendering, media generation, platform account access, analytics API read, scraping, publishing change, autonomous retry, self-improvement mutation, or n8n execution was enabled in this slice.
...
12. Before any real live activation, rehearse Stage 1 locally only with explicit human approval, a Gemini key in secret env, `ALLOW_LIVE_AI_EXECUTION=true`, `LIVE_AI_ACTIVATION_STAGE=1`, a real approved `live_execution.provider_activation` approval ID, kill switches off, and the `/api/live-execution/research/ideation` endpoint. Do not use the generic gateway endpoint for live execution.
