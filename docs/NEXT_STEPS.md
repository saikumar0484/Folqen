...
Current save point: May 13, 2026, after the Controlled Live Execution Activation Layer update. The latest live deployment remains `https://folqen.vercel.app`; no production deploy, live database migration application, credential change, paid tool activation, production provider activation, live AI provider execution, live embeddings, live ComfyUI execution, GPU execution, FFmpeg rendering, platform account access, analytics API read, scraping, publishing change, or n8n execution was enabled in this slice.
...
12. Before any real live activation, add persisted activation registry records or Setting-backed activation state, verify approval IDs against actual approved `Approval` rows, and require fresh policy evaluation at execution time.
...
23. Use `/settings` Connection Wizard for Gemini only after explicit provider approval, then run a local Stage 1 controlled live test only when `ALLOW_LIVE_AI_EXECUTION=true`, `LIVE_AI_ACTIVATION_STAGE=1`, an approved activation ID exists, and the kill switch is off.
