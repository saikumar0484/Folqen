...
Current save point: May 13, 2026, after the governed live Research Department expansion. The latest live deployment remains `https://folqen.vercel.app`; no production deploy, live database migration application, credential change, paid tool activation, production provider activation, live AI provider execution, live embeddings, live ComfyUI execution, GPU execution, FFmpeg rendering, media generation, platform account access, analytics API read, scraping, publishing change, autonomous retry, self-improvement mutation, workflow mutation, or n8n execution was enabled in this slice.
...
12. Before any real live activation, rehearse Stage 1 locally only with explicit human approval, a Gemini key in secret env, `ALLOW_LIVE_AI_EXECUTION=true`, `LIVE_AI_ACTIVATION_STAGE=1`, a real approved `live_execution.provider_activation` approval ID, kill switches off, and either `/api/live-execution/research/ideation` or `/api/live-execution/research/workflows`. Do not use the generic gateway endpoint for live execution.
...
14. Add richer run history/read-model panels for `/research-intelligence`, `/content-studio`, `/organizational-memory`, media assets/renders, platform operations deployments, governance audit/policy history, AI gateway traces, and controlled live execution traces.
...
23. Use `/settings` Connection Wizard or secret env for Gemini only after explicit provider approval, then run a local Stage 1 controlled live Research ideation test only when `ALLOW_LIVE_AI_EXECUTION=true`, `LIVE_AI_ACTIVATION_STAGE=1`, an approved activation ID exists, persisted activation state is enabled, budget/quota gates pass, and the kill switch is off.
24. Configure local or Oracle worker endpoints for FFmpeg, ComfyUI, and TTS, then add test-only connection checks. Live rendering still needs explicit human approval.
25. Continue broader security hardening, including stronger CSRF patterns, server-side action audits, and provider-secret isolation.
...
Continue from branch `build/phase-0-foundation`, read README, all root project docs, and checkpoint docs, run verification, then continue persisted live activation state, approval-ID verification, Google Drive storage, n8n embed/webhook setup, AI provider approval/runtime adapter planning, media worker setup, governance/AI-gateway/live-execution read models, platform-ops read models, posting package polish, service-backed mock APIs, or role-aware UI/tests. All required authenticated pages are route-specific, provider setup panels exist, metadata-only file registration is verified, and safe mock-agent draft creation exists.
